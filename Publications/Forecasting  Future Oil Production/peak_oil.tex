...
\title{Forecasting future oil production}


\author{L. Fiévet, Z. Forró, M. del Degan, P. Cauwels and D. Sornette\\
D-MTEC, ETH Zürich}
...
\maketitle
\begin{abstract}
\noindent This paper presents a new Monte-Carlo methodology to forecast
the crude oil production of Norway and the U.K. based on the current/past
performances of individual oil fields. By extrapolating the future
production of these fields and the frequency of new discoveries, the
oil production of these countries can be forecasted. The results indicate
that standard methodology tends to underestimate remaining oil reserves.
A comparison of the model to this methodology is made by making predictions
with the full data and a back-test starting in 2008. It shows that
the model gives a better description of future oil production.
\end{abstract}
...
century because of oil's central role in our economy. Its importance
ranges from energy production, through manufacturing to pharmaceuticals
...
from Norway and the U.K. which have been reliable exporters. However,
...
was based on the finiteness of oil reserves and use of the logistic
differential equation

\begin{equation}
\frac{dP}{dt}=rP\left(1-\frac{P}{K}\right)\label{logistic}
\end{equation}


\noindent to model oil production. The logistic differential equation
...
oil extracted). If $P(t)$ is the the amount of oil extracted up to
time $t$, then $\frac{dP}{dt}$ is the oil production rate, the quantity
...
Hubbert model (Brecha, Laherrère or Lynch \cite{Brecha 2012,Laherr=0000E8re 2002,Lynch 2002})
or economical model applied to aggregate production (Greiner \cite{Greiner 2012}),
...
data. In this paper, a new methodology is introduces to forecast future
...
oil production is forecasted by the means of a Monte Carlo simulation.
To demonstrate the universality of the methodology presented here,
it is applied to 2 major oil producing countries with publicly available
...
arising from the combination of all the individual field\textquoteright{}s
dynamics. In order to achieve that, one must be able 1) to \textbf{extend
...
the field starts decaying. The decay can take many different shapes
and is governed by a variety of geological and economical factors
which will be captured implicitly by the fitting procedure.
...
show that independently of the country, oil fields can be classified
into 3 main categories:
\begin{itemize}
\item \textbf{Regular fields} - Their decay show some regularity. 
\item \textbf{Irregular fields} - The ones that don't decay in a regular
fashion.
\item \textbf{New fields} - The ones that don't decay yet. As such, there
...
decay dynamics as possible, the stretched exponential

\begin{equation}
P(t)=P_{0}e^{sign(\tau)\left(\frac{t}{\left|\tau\right|}\right)^{\beta}}\label{stretched_exponential}
\end{equation}
has been fitted to the decay of regular fields. The stretched exponential
has many advantages as it generalized the power law and can therefore
capture a broad variety of distributions as shown by Laherrère and
Sornette \cite{Laherr=0000E8re and Sornette 1998}. As can be seen
on figure \ref{regular}, the stretched exponential (equation \ref{stretched_exponential})
is a good functional form to fit the decay process of regular fields.

For the minority of irregular fields, their decay has been modeled
as follows: $\tau$ has been picked to be the average $\tau$ over
the regular fields. Then $\beta$ has been fixed so that the sum of
the field\textquoteright{}s production over its lifetime be equal
...
cannot be extrapolated and will therefore be treated as a new discovery.
The technical details of how to treat them as a new discovery are
...
truncated at a certain date in the past $\tau\in\{0,\,\ldots,\, N\}$.
Where $\tau$ is in months since production start of the field.
\item The extrapolation$f(t)$ is made based on the truncated data $\{p_{0},\,\ldots,\, p_{\tau}\}$.
\item The future production predicted by the extrapolation $f(t)$ can be
compared to the actual production from the date $\tau$ in the past
up to now $\tau_{f}=N$. The extrapolated total production can be
computed as 
\begin{equation}
P_{e}(\tau)=\int_{\tau}^{\tau_{f}}f(t)dt
\end{equation}
and the relative error is given by
\begin{equation}
e(\tau)=\frac{P_{e}(\tau)-\sum_{i=\tau}^{\tau_{f}}p_{i}}{P_{e}(\tau)},
\end{equation}
where both are a function of the truncation time $\tau$.
\end{itemize}
Computing this back test for every month in the past since the field
starting decaying, yields a plot showing the evolution of the relative
error over time \ref{oesberg-error}. By construction, the relative
error will tend to zero as the truncation time $\tau$ goes to the
present. Nonetheless, it is a uself indicator for the stability of
the extrapolation. As can been seen for the Oseberg field, the relative
error on future production remained fairly stable during the past
decade.

From the complete back-test, on can compute the average relative error
\begin{equation}
\bar{e}=\frac{1}{N}\sum_{i=0}^{N}e(i)
\end{equation}
 the extrapolation made on the future production. Assuming that the
...
was corrected by the average relative error. Meaning that if the extrapolation
...
production $p(t)$, including a 1$\sigma$ confidence interval, as
\begin{equation}
p(t)=(1-\bar{e})\, f(t)\pm\sigma_{e}.\label{eq:extrapolation}
\end{equation}


An example of such an extrapolation including a one standard deviation
range is shown on figure \ref{irregular} for the Oesberg field.


\subsubsection{Aggregate error}

Once the individual fields have been extrapolated in the sense of
formula \ref{eq:extrapolation}, the aim is to compute the extrapolation
for the country. While it is straightforward to sum the extrapolations
of the individual fields to obtain the expected production, some care
has to be taken with respect to the confidence interval on the production
...
the relative error between individual fields is uncorrelated. Therefore,
...
This means that the risk at the country level from the extrapolated
production is to be computed as 
\begin{equation}
...
at a rate $\lambda$. It is inhomogeneous if the rate is time-dependent,
...
Then, $\lambda(t)$ is simply $\frac{dN(t)}{dt}$. Figure \ref{rate_of_discovery}
shows $N(t)$ for Norwegian fields classified according to their size.
...
by Forro \cite{Forro 2012} to estimate the number of daily active
users on Zynga.
...
can not be determined if the data does not already exhibit the slow
down in growth from reaching the carrying capacity. However, it is
...
for the dwarfs is poorly constrained. As can be seen on figure \ref{rate_of_discovery},
...
$K$ can lead to an equally good fit of the data. Therefore, a method
described in section \ref{sub:Likelihood-function-for} has been used
...
To overcome the logistic fitting issue for the dwarf fields, a method
...
discovered before the $i^{th}$ step. Than the probability that the
discovery at step $i^{th}$ is of size $j$ can be expressed as
\begin{equation}
...
For the analysis of the north sea oil field discoveries, the number
...
is unlikely and that future discoveries wil mostly be made up of dwarf
...
The goal is to compute an expected oil production coming from future
...
of future discoveries. The aim is to use the likelihood function to
...
fields) is given for each size class. This is useful, because it resolves
...
\begin{enumerate}
\item Draw a scenario based on its probability.
...
In order to account for the new fields discussed in section \ref{sub:Regular,-irregular-and},
which are already discovered but did not yet enter the decay phase,
the simulated production resulting from new discoveries has been shifted
to the date in the past where it matches the current production from
new fields.
...
the existing fields were extrapolated and the future discoveries were
simulated. Figure \ref{no_uk_2013} shows the average of 1000 simulations.
For each country, the non-symmetric shape of the production's dynamics
contrary to what Hubbert would have predicted is immediately noticeable.

\begin{table}[H]
\caption{Remaining oil reserves until 2030 in barrels for the \textbf{Fit}
of the country's production, the Monte-Carlo \textbf{Model} and the
relative difference $\Delta=\frac{\textrm{Model}-\textrm{Fit}}{\textrm{Model}}$
between the two.}
...
between the fit and the Monte-Carlo model forecast. According to the
...
different effects:
\begin{itemize}
\item The sum of the forecast of the individual existing fields is higher
...
However, this has not been an issue to extrapolate the decay of production
...
Which of the two models is more thrust worthy? Clearly, the implications
...
\textquotedbl{}What would have each of the models predicted had they
been used in the past?\textquotedbl{}
...
shows the outcome of those tests. Comparing the forecast of both models
with the oil production of the subsequent 6 years, shows that the
predictive power of the Monte-Carlo model is better than a simple
extrapolation of aggregate production. The following table \ref{backtest-08}
summarizes the difference between the two approaches for the back-testing
period.

\begin{table}[H]
\caption{The fit and the Monte-Carlo model are used on the data set truncated
in 2008 and their forecast for the period 2008-2014 is compared to
...
The difference is not huge for the period from 2008 to 2014, but the
Monte-Carlo model nevertheless is noticeably closer to the actual
value. However, when considering the period after 2014, the difference
is huge as shown in table \ref{forecast-08}.

\begin{table}[H]
\caption{Remaining oil reserves forecasted for the period 2014-2030 when using
the data truncated in 2008.}
...
As well, it must be noted that the simple extrapolation changed massively
...
U.K.). In other words, the simple extrapolation was very unstable
in its forecast, while the Monte-Carlo forecasts remained very consistent
(less than 10\% change).
...
This paper presented a Monte-Carlo based methodology to forecast future
...
The results are hopeful, as they make it highly likely that the decay
...
Norway is a lower bound based on the changes in the U.K and Norway.}
...
E.U.+Norway (lower bound) & $9.8\cdot10^{6}$  & $10.45\cdot10^{6}$  & $10.85\cdot10^{6}$  & $11.1\cdot10^{6}$ \tabularnewline
\bottomrule  &  &  &  & \tabularnewline
...
its oil imports by $1.3$ million barrels. Imports which will most
likely have to come from outside Europe.
...
\bibitem{Forro 2012}Forró, Zalán, Peter Cauwels, and Didier Sornette.
...
\bibitem{H=0000F6=0000F6k 2008}Höök, Mikael, and Kjell Aleklett.
...
\bibitem{Laherr=0000E8re 2002}Laherrere, Jean. \textquotedblleft{}Forecasting
...
\bibitem{Laherr=0000E8re and Sornette 1998}Laherrere, Jean, and Didier
...
\caption{Example of a regular field with $\beta=0.66\pm0.01$ and $\tau=-55\pm1$.}
...
\caption{Oesberg field - Relative error on predicted total production from
time $t$ until 2014}
\label{oesberg-error}
\end{figure}
\begin{figure}[p]
\includegraphics[width=1\columnwidth]{extrapolating-no-discoveries}
\caption{Logistic fit to the number of discoveries for Norway. The discovery
rate of new oil fields is dependent on their size.}
...
\includegraphics[width=1\columnwidth]{UK-future} \caption{Monte-Carlo and fit forecast based on past production data for Norway
(top) and the U.K. (bottom). In both cases, the Mont-Carlo model forecasts
a significantly slower decay than the fit.}
...
\includegraphics[width=1\columnwidth]{UK-back-test} \caption{Monte-Carlo and Hubbert forecast based on past production data up
to 2008 for Norway (top) and the U.K. (bottom). The results can be
